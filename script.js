...
const WeatherAPI = (() => {
    ...
    async function fetchByCity(city) {
        ...
        if (!res.ok) throw new Error(`Nie znaleziono miasta „${city}” (HTTP ${res.status}).`);
    ...
    function demoData(city) {
        ...
        return { city: city || "Twoje miasto", ...s, demo: true };
    }
...
const ChatUI = (() => {
    ...
    function greet() {
        botReply(
            ...
            "Opisz pogodę (np. „Jest 7 stopni i pada deszcz”), a doradzę Ci, " +
            "jak się ubrać. Możesz też kliknąć 📍, by pobrać pogodę dla swojego miasta.",
            400
        );
    }
    ...
    async function handleCityWeather(city) {
        ...
        try {
            ...
            const demoNote = data.demo ? " (dane demonstracyjne — dodaj klucz API)" : "";
            addMessage(
